...
from utils.chem import clean_mol
...
from utils.reaction_actions import AddGroupAction, AtomEditAction, BondEditAction, Termination
...
class BeamSearch:
    ...
    def run(self, prod_smi: str, max_steps: int = 8, rxn_class: int = None) -> dict:
        product = Chem.MolFromSmiles(prod_smi)
        Chem.Kekulize(product)
        prod_graph = MolGraph(mol=Chem.Mol(
            product), rxn_class=rxn_class, use_rxn_class=self.use_rxn_class)
        ...
        for step_i in range(max_steps):
            ...
            for path in followed_path:
                try:
                    ...
                    # print(f'Exception while process the one step path: {str(e)}')
                    continue
                ...
                try:
                    ...
                    for i, edit in enumerate(path['edits']):
                        ...
                        if edit == 'Terminate':
                            ...
                            [a.ClearProp('molAtomMapNumber')
                             for a in pred_mol.GetAtoms()]
                            final_smi = Chem.MolToSmiles(pred_mol, isomericSmiles=True)
                            path['final_smi'] = final_smi
        ...
        for beam_idx, path in enumerate(finished_paths):
            pred_smi = path['final_smi']
            pred_smi = clean_mol(pred_smi)
            prob = path['prob']
            str_edits = '|'.join(f'({str(edit)};{p})' for edit, p in zip(
                path['rxn_actions'], path['edits_prob']))

            if pred_smi is not None and pred_smi not in reactants:
                reactants.append(pred_smi)
                scores.append(prob)
                reaction_edits.append(str_edits)

        out_dict = {'reactants': reactants,
                    'scores': scores,
